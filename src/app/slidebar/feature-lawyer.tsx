...
export default function FeaturedLawyer() {
  ...
  useEffect(() => {
    const fetchLawyers = async () => {
      try {
        ...
        // data.users is the actual array of users
        const subscribedLawyers = (data.users || []).filter(
          (lawyer) => lawyer.subscribe === true && lawyer.islaywer === true
        );
  ...
  // Auto-switch featured lawyer every 15 sec (FAST, for testing — adjust to 12hr later)
  useEffect(() => {
    if (lawyers.length > 1) {
      const timer = setInterval(() => {
        setCurrentIndex((prev) => (prev + 1) % lawyers.length);
      }, 15000); // 15 sec (change to 12hr = 43200000 ms for prod)
  ...
  if (lawyers.length === 0) {
    return <p className="text-center text-white">No featured lawyers available.</p>;
  }
      ...
      <div className="md:flex items-center">
        <div className="md:flex-shrink-0 relative w-full md:w-72 h-72 rounded-lg overflow-hidden shadow-lg">
          <Image
            ...
            alt={`${currentLawyer.user}'s profile`}
            layout="fill"
            objectFit="cover"
        ...
        <div className="p-6 text-white">
          ...
          <h2 className="mt-2 text-4xl font-bold text-[#FFFFFF]">
            {currentLawyer.name || "Unknown"}
          </h2>
          <p className="mt-4 text-lg text-[#E0E7FF]">
            {currentLawyer.name || "Unnamed"} is a highly skilled attorney with a strong track record...
          </p>

          <div className="mt-6 grid grid-cols-3 gap-4">
            <StatCard label="Location" value={currentLawyer.location} />
            <StatCard label="Notable Cases" value={currentLawyer.recentCases?.length || 0} />
            <StatCard label="Awards" value={currentLawyer.awards?.length || 0} />
          </div>

          <div className="mt-8 flex space-x-4">
            <Link
              href={`/userid/${currentLawyer._id}`}
              className="flex-1 bg-gradient-to-r from-[#2563EB] to-[#3B82F6] hover:from-[#1D4ED8] hover:to-[#2563EB] text-white font-bold py-2 px-4 rounded-lg shadow-md transition-all text-center"
            >
              View Profile
            </Link>
            <Link
...
function StatCard({ label, value }) {
  return (
    <div className="bg-[#162447] p-4 rounded-lg text-center shadow-md">
      ...
      <div className="text-sm text-[#E0E7FF]">{label}</div>
    </div>
  );
}
